...
export default [
  {
    ...
    Controller: class extends ControllerSuperclass {
      async handler () {
        const { renders: r, services: s, payloads: p, apis: a } = this
        try {
          ...
          const payload = await a.cron.self(task.path)

          await s.cronTasks.setTwitchApiNext(cron, task)
          await s.cronTasks.setTaskInterval(task, payload)
